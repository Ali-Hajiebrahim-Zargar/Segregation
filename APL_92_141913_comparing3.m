...
temp=643.15;                      %growth temperature in kelvin
...
X_s_In_0=0;                       %initial condition
X_s_Ga_0=0;                       %initial condition
temp=693.15;                      %growth temperature in kelvin
phi_Ga=0.64*growth_rate;           %effects of different growth rate
phi_In=0.36*growth_rate;           %effects of different growth rate
v=10^13;                          %Vibration Frequency for all semiconductors
p1=v*exp(-E1/(kB*temp));
...
X_s_In_0=0;                       %initial condition
X_s_Ga_0=0;                       %initial condition
temp=743.15;                      %growth temperature in kelvin
phi_Ga=0.64*growth_rate;           %effects of different growth rate
phi_In=0.36*growth_rate;           %effects of different growth rate
v=10^13;                          %Vibration Frequency for all semiconductors
p1=v*exp(-E1/(kB*temp));
...
ml=[-1:1:ML+1];                                          %print X_b_In for each monolayer
plot(time,discrete,'r-','linewidth',0.5)
title('In0.2Ga0.8As/GaAs growth rate=1: temp=500 & temp=400')
xlabel('Thickness(ML)')
ylabel('In concentration')
axis([-1 27 0 0.4])
...
ml=[-1:1:ML+1];                                          %print X_b_In for each monolayer
plot(time,discrete,'b-','linewidth',0.5)
title('In0.36Ga0.64As/GaAs growth rate=0.1')
xlabel('Thickness(ML)')
ylabel('In concentration')
axis([-1 27 0 0.4])
hold on
legend('temp=370','temp=420','temp=470','temp=520')